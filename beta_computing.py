...
def unnormalization(df,names,stat_list):
  df_unnormalized = df.copy()
  for name in names:
    mean = stat_list[name][0]
    std = stat_list[name][1]
    df_unnormalized[[name]] = (df_normalized[[name]]*std)+mean
...
kmeans = KMeans(n_clusters=2, random_state=0).fit(normalized_data_np)
